...
namespace PortableDeviceLib
{
    ...
    public abstract class PathEnumerateHelperBase : IObjectEnumerateHelper
    {
        protected Queue<string> paths;

        public PathEnumerateHelperBase(string path)
        {
            paths = new Queue<string>(path.Split(new[] { '/' }, StringSplitOptions.RemoveEmptyEntries));
        }

        protected PathEnumerateHelperBase(Queue<string> paths)
        {
            this.paths = paths;
        }
        ...
        public abstract IObjectEnumerateHelper Next();
    ...
    public class PathEnumerateHelper : PathEnumerateHelperBase
    {
        public override bool IsLastNode
        {
            get { return paths.Count <= 1; }
        }

        public PathEnumerateHelper(string path) : base(path)
        {
        }

        private PathEnumerateHelper(Queue<string> paths) : base(paths)
        {
        }

        public override IObjectEnumerateHelper Next()
        {
    ...
    [DebuggerDisplay("Count = {paths.Count}")]
    public class RecursivePathEnumerateHelper : PathEnumerateHelperBase
    {
        public override bool IsLastNode
        {
            get { return false; }
        ...
        public RecursivePathEnumerateHelper(string path)
            : base(path)
        {
        }

        private RecursivePathEnumerateHelper(Queue<string> paths)
            : base(paths)
        {
        }
        ...
        public override IObjectEnumerateHelper Next()
        {
            var deq = new Queue<string>(paths);
            deq.Dequeue();
            return new RecursivePathEnumerateHelper(deq);
        }
